...
use PPP_L8\Illuminate\Support\Enumerable;
use PPP_L8\Illuminate\Support\HigherOrderCollectionProxy;
use PPP_L8\Illuminate\Support\HigherOrderWhenProxy;
...
trait EnumeratesValues
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (func_num_args() === 2) {
            return $this->contains(function ($item) use ($key, $value) {
                return PPP_L8_data_get($item, $key) === $value;
            });
        }
    ...
    public function whereIn($key, $values, $strict = false)
    {
        ...
        return $this->filter(function ($item) use ($key, $values, $strict) {
            return in_array(PPP_L8_data_get($item, $key), $values, $strict);
        });
    }
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use ($key, $values) {
            return PPP_L8_data_get($item, $key) < reset($values) || PPP_L8_data_get($item, $key) > end($values);
        });
    }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use ($key, $operator, $value) {
            $retrieved = PPP_L8_data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use ($value) {
            return PPP_L8_data_get($item, $value);
        };
    }
